...
const firebaseServiceFavorites = {
  ...
  getDocumentByIdUser: async (collectionName, iduser) => {
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        const documents = querySnapshot.docs.map(doc => ({ id: doc.id, idmovie:doc.data().idmovie }));
  ...
  //READ
  getDocumentById: async (collectionName, iduser, idmovie) => {
    try {
      //obtengo el doc de la relacion
      //const coll = collection(db, collectionName);
      const q = query(collection(db, collectionName), where('idmovie', '==', idmovie), where('iduser', '==', iduser));
      const querySnapshot = await getDocs(q);
